...
export class FavoriteCities {
  ...
  createCityElement(city, currentWeather) {
    ...
    const convertedTemp = Math.round(
      weatherStore.convertTemperature(city.temp, units)
    );
    const unitSymbol = units === "metric" ? "°C" : "°F";
    tempElement.textContent = `${convertedTemp}${unitSymbol}`;
    ...
    loadButton.appendChild(
      Icon.create("location", {
        size: 16,
        className: "favorite-city__load-icon",
      })
    );
  ...
  renderEmpty() {
    ...
    hint.textContent =
      "Добавьте город в избранное, нажав на звезду в карточке погоды";
